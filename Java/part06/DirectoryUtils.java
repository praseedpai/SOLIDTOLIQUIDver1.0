...
public class DirectoryUtils {
   ...
   public static ArrayList<FILE_INFO> GetAllFilesWithExt(String dir,String ext)
   {

        return   GetAllFilesasCollection(dir).stream().filter(
                  s -> s.name.endsWith(ext)).collect(Collectors.toCollection(ArrayList<FILE_INFO>::new));

   }

   public static ArrayList<FILE_INFO> GetAllFilesLessThan(String dir,long sizemax)
   {

        return   GetAllFilesasCollection(dir).stream().filter(
                  s -> s.size < sizemax).collect(Collectors.toCollection(ArrayList<FILE_INFO>::new));
